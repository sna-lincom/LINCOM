...
int main( int argc, char **argv)
{
  ...
  for(int start_node=1; start_node<=max_label; start_node++) {
    printf("start_node = %d\n", start_node);
    printf("degree of start_node = %d\n", adlist[start_node][0]);
    ...
    node start;
    start.label=start_node;
    start.score=0;
    start.parent=start.label;
    start.community=start_node;
    ...
    sprintf(cmd, "%s %s %d", "./script_start_node.sh", filename, start_node);
    system(cmd);
